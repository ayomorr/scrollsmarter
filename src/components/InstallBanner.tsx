...
export function InstallBanner() {
  ...
  return (
    <div className="fixed bottom-[88px] md:bottom-6 inset-x-3 md:left-1/2 md:-translate-x-1/2 md:max-w-[420px] z-50">
      <div className="bg-[#17152B] text-white rounded-[20px] p-4 flex items-center gap-3 shadow-[0_12px_32px_rgba(0,0,0,0.22)] border border-white/10">
        <div className="w-10 h-10 rounded-xl bg-[#FF4F87] grid place-items-center shrink-0">
        ...
        {canInstall ? (
          <button onClick={()=> prompt()} className="bg-white text-[#17152B] px-4 py-2 rounded-full text-sm font-semibold shrink-0">Install</button>
        ) : (
          <a href="#install" onClick={dismiss} className="bg-white text-[#17152B] px-4 py-2 rounded-full text-sm font-semibold shrink-0">How</a>
        )}
