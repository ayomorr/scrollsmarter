...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#FDF8F1] dark:bg-[#121412] text-[#1A1E1D] dark:text-[#F5F1EB] flex flex-col max-w-[480px] mx-auto md:border-x border-black/5 dark:border-white/5 md:shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
      {/* Top bar */}
      <div className="sticky top-0 z-30 bg-[#FDF8F1]/80 dark:bg-[#121412]/80 backdrop-blur-xl border-b border-black/5 dark:border-white/5 pt-[env(safe-area-inset-top)]">
        <div className="h-[56px] px-4 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-[#5B7A5F] grid place-items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isStandalone && (
              <Link to="/#install" className="text-xs font-semibold px-3 py-1.5 rounded-full bg-[#1A1E1D] dark:bg-white text-white dark:text-black">Install</Link>
            )}
          </div>
        </div>
        {showUpdate && (
          <div className="mx-3 mb-3 bg-[#5B7A5F] text-white rounded-2xl px-4 py-3 flex items-center justify-between gap-3" role="alert" aria-live="polite">
            ...
            }} className="bg-white text-[#5B7A5F] px-4 py-1.5 rounded-full text-sm font-bold shrink-0">Refresh</button>
          </div>
        )}
      </div>
