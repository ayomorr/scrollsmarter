...
export default function Home() {
  ...
  return (
    <div className="px-4 pt-4 space-y-4">
      {/* Progress card */}
      <div className="bg-white dark:bg-[#1E2320] rounded-[24px] p-5 border border-black/5 dark:border-white/5 shadow-sm">
        <div className="flex items-start justify-between">
          <div>
            <div className="text-[11px] font-bold tracking-[0.14em] uppercase text-[#5B7A5F]">Today's progress</div>
          ...
          <div className="text-right">
            <div className="w-14 h-14 rounded-full border-[3px] border-[#E8F0E8] dark:border-white/10 grid place-items-center relative">
              <div className="absolute inset-0 rounded-full" style={{ background: `conic-gradient(#5B7A5F ${pct}%, transparent ${pct}%)` }} />
              <div className="w-[46px] h-[46px] rounded-full bg-white dark:bg-[#1E2320] grid place-items-center text-xs font-bold">{pct}%</div>
            </div>
        ...
        <div className="mt-4 h-2 bg-black/5 dark:bg-white/10 rounded-full overflow-hidden">
          <div className="h-full bg-[#5B7A5F] transition-all" style={{ width: `${pct}%` }} />
        </div>
        <div className="mt-3 flex gap-2 text-xs">
          <span className="px-2.5 py-1 rounded-full bg-[#F5EBDD] dark:bg-white/10 font-medium">{sessions.filter(s=> new Date(s.startedAt).toISOString().slice(0,10)===new Date().toISOString().slice(0,10)).length} sessions</span>
          {inQuiet && <span className="px-2.5 py-1 rounded-full bg-[#1A1E1D] text-white dark:bg-white dark:text-black font-medium">Quiet hours</span>}
        </div>
      </div>

      {/* Active session */}
      {active ? (
        <div className="bg-[#1A1E1D] dark:bg-white text-white dark:text-[#1A1E1D] rounded-[24px] p-5 relative overflow-hidden">
          <div className="absolute -right-8 -top-8 w-32 h-32 bg-[#5B7A5F]/30 blur-2xl rounded-full" />
          <div className="relative">
            ...
            <div className="mt-4 h-1.5 bg-white/15 dark:bg-black/10 rounded-full overflow-hidden">
              <div className="h-full bg-white dark:bg-[#5B7A5F] transition-all" style={{ width: `${((active.totalSeconds-active.remaining)/active.totalSeconds)*100}%` }} />
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              {!active.pausedAt ? (
                <button onClick={pauseSession} className="py-3 rounded-full bg-white dark:bg-[#1A1E1D] text-[#1A1E1D] dark:text-white font-semibold text-sm">Pause</button>
              ) : (
              ...
              <button onClick={()=> endSession('completed')} className="py-3 rounded-full bg-[#5B7A5F] text-white font-semibold text-sm">I'm done</button>
            </div>
        ...
        <div className="bg-white dark:bg-[#1E2320] rounded-[24px] p-6 border border-black/5 dark:border-white/5 text-center">
          <div className="w-12 h-12 rounded-full bg-[#F5EBDD] dark:bg-white/10 grid place-items-center mx-auto text-xl">⏳</div>
          ...
          <button onClick={()=> setShowStart(true)} className="mt-4 w-full py-3.5 rounded-full bg-[#5B7A5F] text-white font-semibold">Start session</button>
      ...
      {!active && (
        <div>
          <div className="flex items-center justify-between">
            ...
            <button onClick={()=> setShowStart(true)} className="text-sm font-medium text-[#5B7A5F]">Customize →</button>
          </div>
          <div className="grid grid-cols-3 gap-3 mt-3">
            {[
              ...
              <button key={q.label} onClick={()=> { setMinutes(q.m); setPurpose(q.label); setCategory(q.cat); startSession(q.label, q.cat, q.m)}} className="bg-white dark:bg-[#1E2320] rounded-2xl p-4 border border-black/5 dark:border-white/5 text-center hover:border-[#5B7A5F]/20 transition">
      ...
      {/* Limits */}
      <div className="bg-white dark:bg-[#1E2320] rounded-[24px] p-5 border border-black/5 dark:border-white/5">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Your limits</h3>
          <span className="text-xs bg-[#F5EBDD] dark:bg-white/10 px-2.5 py-1 rounded-full font-medium">Self-set</span>
        </div>
              ...
              <div key={l.id} className="flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <div className="mt-1.5 h-1.5 bg-black/5 dark:bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-[#5B7A5F]" style={{ width: `${pctCat}%` }} />
                  </div>
                </div>
      ...
      {showStart && (
        <div className="fixed inset-0 z-50 grid place-items-end sm:place-items-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-[480px] bg-[#FDF8F1] dark:bg-[#1E2320] rounded-t-[28px] sm:rounded-[28px] p-6 pb-[calc(16px+env(safe-area-inset-bottom))] border-t border-black/5 max-h-[92dvh] overflow-y-auto shadow-[0_-12px_40px_rgba(0,0,0,0.12)]">
            ...
            <input value={purpose} onChange={e=> setPurpose(e.target.value)} placeholder="e.g., Catch up with friends" className="mt-1.5 w-full px-4 py-3 rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-white/5 focus:outline-none focus:ring-2 focus:ring-[#5B7A5F]/20" />

            <label className="block mt-4 text-sm font-medium">Category</label>
            <div className="mt-1.5 grid grid-cols-3 gap-2">
              {['Social media','Video','News','Learning','Other'].map(c=>(
                <button key={c} onClick={()=> setCategory(c)} className={`px-3 py-2.5 rounded-full border text-sm font-medium transition ${category===c ? 'bg-[#5B7A5F] text-white border-[#5B7A5F]' : 'bg-white dark:bg-white/5 border-black/10 dark:border-white/10'}`}>{c}</button>
              ))}
            </div>
            ...
            <div className="mt-2 grid grid-cols-4 gap-2">
              {[5,10,15,20,25,30,45,60].map(m=>(
                <button key={m} onClick={()=> setMinutes(m)} className={`py-3 rounded-2xl border font-semibold ${minutes===m ? 'bg-[#1A1E1D] text-white dark:bg-white dark:text-black border-transparent' : 'bg-white dark:bg-white/5 border-black/10 dark:border-white/10'}`}>{m}m</button>
              ))}
            </div>
            <div className="mt-3 flex items-center gap-3">
              <input type="range" min={2} max={60} value={minutes} onChange={e=> setMinutes(Number(e.target.value))} className="flex-1 accent-[#5B7A5F]" />
              <span className="text-sm font-semibold w-14 text-right">{minutes} min</span>
            </div>

            <button onClick={handleStart} className="mt-6 w-full py-3.5 rounded-full bg-[#5B7A5F] text-white font-semibold text-[16px]">Start {minutes}-minute session</button>
