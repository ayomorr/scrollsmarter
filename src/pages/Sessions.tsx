  ...
  return (
    <div className="px-4 pt-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={()=> setShowNew(true)} className="px-4 py-2 rounded-full bg-[#FF4F87] text-white text-sm font-semibold">New</button>
      </div>

      <div className="flex gap-2">
        <button onClick={()=> setFilter('all')} className={`px-4 py-2 rounded-full text-sm font-medium border ${filter==='all' ? 'bg-[#17152B] text-white dark:bg-white dark:text-black border-transparent' : 'bg-white dark:bg-white/5 border-black/10 dark:border-white/10'}`}>All</button>
        <button onClick={()=> setFilter('today')} className={`px-4 py-2 rounded-full text-sm font-medium border ${filter==='today' ? 'bg-[#17152B] text-white dark:bg-white dark:text-black border-transparent' : 'bg-white dark:bg-white/5 border-black/10 dark:border-white/10'}`}>Today</button>
      ...
      <div className="space-y-3">
        {list.length===0 ? (
          <div className="bg-white dark:bg-[#26213B] rounded-[20px] p-8 text-center border border-black/5 dark:border-white/5">
            <div className="w-12 h-12 rounded-full bg-[#F0DFDA] dark:bg-white/10 grid place-items-center mx-auto">📝</div>
            ...
            <button onClick={()=> setShowNew(true)} className="mt-4 px-6 py-2.5 rounded-full bg-[#FF4F87] text-white text-sm font-semibold">Start one</button>
          </div>
        ) : list.map(s=> {
          const d = new Date(s.startedAt)
          const minsSpent = s.remainingSeconds != null ? Math.ceil((s.durationMinutes*60 - s.remainingSeconds)/60) : s.durationMinutes
          return (
            <div key={s.id} className="bg-white dark:bg-[#26213B] rounded-2xl p-4 border border-black/5 dark:border-white/5 flex gap-3">
              <div className={`w-10 h-10 rounded-xl grid place-items-center shrink-0 ${s.status==='completed' ? 'bg-[#DFF8F5] dark:bg-emerald-900/20' : s.status==='cancelled' ? 'bg-stone-100 dark:bg-white/5' : 'bg-amber-100'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-xs font-semibold bg-[#F0DFDA] dark:bg-white/10 px-2 py-1 rounded-full">{s.durationMinutes} min planned</span>
      ...
      {showNew && (
        <div className="fixed inset-0 z-50 grid place-items-end sm:place-items-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-[480px] bg-white dark:bg-[#26213B] rounded-t-[24px] sm:rounded-[24px] p-6 border-t border-black/5 max-h-[90dvh] overflow-auto">
            <h3 className="font-display text-xl">New session</h3>
            <label className="block text-sm font-medium mt-4">Purpose</label>
            <input value={purpose} onChange={e=> setPurpose(e.target.value)} placeholder="What will you do?" className="mt-1 w-full px-4 py-3 rounded-2xl border border-black/10 dark:border-white/10 bg-[#FFF7F3] dark:bg-white/5" />
            <label className="block text-sm font-medium mt-4">Category</label>
            <div className="grid grid-cols-3 gap-2 mt-1">
              {['Social media','Video','News','Learning','Other'].map(c=>(
                <button key={c} onClick={()=> setCat(c)} className={`px-3 py-2 rounded-full text-sm font-medium border ${cat===c?'bg-[#FF4F87] text-white border-[#FF4F87]':'bg-white dark:bg-white/5 border-black/10'}`}>{c}</button>
              ))}
            </div>
