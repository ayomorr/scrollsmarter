...
export default function Settings() {
  ...
  return (
    <div className="px-4 pt-4 space-y-4 pb-6">
      ...
      {/* Appearance */}
      <div className="bg-white dark:bg-[#26213B] rounded-[20px] p-5 border border-black/5 dark:border-white/5">
        <h3 className="font-semibold">Appearance</h3>
        <div className="mt-3 grid grid-cols-3 gap-2">
          {(['system','light','dark'] as const).map(m=>(
            <button key={m} onClick={()=> updateSettings({ appearance: m })} className={`py-2.5 rounded-full border text-sm font-medium capitalize ${settings.appearance===m ? 'bg-[#17152B] text-white dark:bg-white dark:text-black border-transparent' : 'bg-white dark:bg-white/5 border-black/10 dark:border-white/10'}`}>{m}</button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#26213B] rounded-[20px] p-5 border border-black/5 dark:border-white/5">
        ...
        <div className="mt-4 flex items-center gap-3">
          <input type="range" min={15} max={180} step={5} value={settings.dailyGoal} onChange={e=> updateSettings({ dailyGoal: Number(e.target.value) })} className="flex-1 accent-[#FF4F87]" />
          <span className="w-16 text-center font-semibold bg-[#F0DFDA] dark:bg-white/10 rounded-full py-1.5 text-sm">{settings.dailyGoal} min</span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#26213B] rounded-[20px] p-5 border border-black/5 dark:border-white/5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={()=> updateSettings({ quietHours: { ...settings.quietHours, enabled: !settings.quietHours.enabled }})} className={`w-11 h-6 rounded-full p-1 transition ${settings.quietHours.enabled ? 'bg-[#FF4F87]' : 'bg-black/10 dark:bg-white/15'}`}>
        ...
        {settings.quietHours.enabled && (
          <div className="mt-4 grid grid-cols-2 gap-3">
            <label className="text-sm">
              ...
              <input type="time" value={settings.quietHours.start} onChange={e=> updateSettings({ quietHours: { ...settings.quietHours, start: e.target.value }})} className="w-full px-3 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-[#FFF7F3] dark:bg-white/5" />
            </label>
            <label className="text-sm">
              <span className="block text-xs font-bold tracking-widest uppercase text-stone-500 mb-1">End</span>
              <input type="time" value={settings.quietHours.end} onChange={e=> updateSettings({ quietHours: { ...settings.quietHours, end: e.target.value }})} className="w-full px-3 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-[#FFF7F3] dark:bg-white/5" />
            </label>
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-[#26213B] rounded-[20px] p-5 border border-black/5 dark:border-white/5">
        ...
        <div className="mt-4 space-y-3">
          {settings.limits.map(l=>(
            <div key={l.id} className="flex items-center gap-3">
              ...
              }} className="w-20 px-3 py-2 rounded-full border border-black/10 dark:border-white/10 bg-[#FFF7F3] dark:bg-white/5 text-center text-sm font-semibold" />
        ...
        <div className="mt-4 grid grid-cols-[1fr_auto_auto] gap-2 items-end">
          <label className="text-xs font-medium">
            ...
            <input value={newLabel} onChange={e=> setNewLabel(e.target.value)} placeholder="e.g., Reading" className="w-full px-3 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-[#FFF7F3] dark:bg-white/5 text-sm" />
          </label>
          <label className="text-xs font-medium">
            <span className="block mb-1 text-stone-500">Min</span>
            <input type="number" min={5} max={180} value={newMins} onChange={e=> setNewMins(Number(e.target.value))} className="w-20 px-3 py-2.5 rounded-full border border-black/10 dark:border-white/10 bg-[#FFF7F3] dark:bg-white/5 text-center text-sm font-semibold" />
          </label>
          <button onClick={()=> {
            ...
            setNewLabel('')
            setNewMins(30)
          }} className="px-4 py-2.5 rounded-full bg-[#FF4F87] text-white text-sm font-semibold h-[44px]">Add</button>
        </div>
      ...
      <div className="bg-white dark:bg-[#26213B] rounded-[20px] p-5 border border-black/5 dark:border-white/5">
        ...
        <div className="mt-3 flex gap-2">
          <button onClick={requestNotif} className={`flex-1 py-2.5 rounded-full font-semibold text-sm ${settings.notifications ? 'bg-[#DFF8F5] text-[#FF4F87] border border-[#FF4F87]/20' : 'bg-[#FF4F87] text-white'}`}>
        ...
        <div className="mt-3 flex gap-3 text-xs">
          <label className="flex items-center gap-2">
            <input type="checkbox" checked={settings.sound} onChange={e=> updateSettings({ sound: e.target.checked })} className="accent-[#FF4F87]" /> Sound
          </label>
          <label className="flex items-center gap-2">
            <input type="checkbox" checked={settings.haptics} onChange={e=> updateSettings({ haptics: e.target.checked })} className="accent-[#FF4F87]" /> Haptics
          </label>
        </div>
      </div>
      ...
      {/* Install */}
      <div className="bg-[#17152B] dark:bg-white rounded-[20px] p-5 text-white dark:text-[#17152B]">
        ...
        <div className="mt-3 flex gap-2">
          {canInstall ? (
            <button onClick={()=> prompt()} className="flex-1 py-2.5 rounded-full bg-white text-[#17152B] dark:bg-[#17152B] dark:text-white font-semibold text-sm">Install now</button>
          ) : (
      ...
      {showReset && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#302A4B] rounded-[24px] p-6 w-full max-w-[360px] border border-black/5 shadow-[0_20px_60px_rgba(0,0,0,0.2)]">
